...
def _compute_feats_sinGabor(image):

	feats = np.zeros(15, dtype=np.double)
	for chanel in range(3):
		filtered = image[:, :, chanel]
		bins32 = np.array(
			[0, 8, 16, 24, 32, 40, 48, 56, 64, 72, 80, 88, 96, 104, 112, 120, 128, 136, 144, 152, 160,
			 168, 176, 184, 192, 200, 208, 216, 224, 232, 240, 248, 255])  # 32-bit
		np.clip(filtered, 0, 255, out=filtered)
		...
		max_value = inds.max() + 1
		matrix_coocurrence = graycomatrix(inds, [1], [0], levels=max_value, normed=False, symmetric=False)
		feats[(chanel * 5 + 0)] = _contrast_feature(matrix_coocurrence)
		feats[(chanel * 5 + 1)] = _dissimilarity_feature(matrix_coocurrence)
		feats[(chanel * 5 + 2)] = _homogeneity_feature(matrix_coocurrence)
		feats[(chanel * 5 + 3)] = _energy_feature(matrix_coocurrence)
		feats[(chanel * 5 + 4)] = _correlation_feature(matrix_coocurrence)

	return feats

def _train_model(model, training_dataset, params, withPca=False):
...
def train_model_tfrecords(ratas_selected, dict_videos, directory, video_test, n_imagenes_x_class_tr, n_imagenes_x_class_te, params, withPca=False):
	...
	model, pca = _train_model(model, training_dataset, params, withPca)

	return model, pca, test_dataset

def test_model(model, test_dataset, pca = None):
	...
	for f1 in x_test:
		...
		#x_test_gabor.append(_compute_feats_sinGabor(f1))
